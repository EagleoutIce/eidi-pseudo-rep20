...
\tikzset{MTX@BLK/.style={rectangle,minimum size=\mtx@k,inner sep=\z@,outer sep=\z@,fill=#1,draw=@primary,line width=.66\p@}}
...
\foreach[count=\y] \line in {#1} {
    \foreach[count=\x] \tar/\gol/\ded in \line {
        ...
        \else
            ...
            \only<\tar>{
                \node[MTX@BLK=@alternative!80!white,text=white] (\x-\y) at (\x*\mtx@k, -\y*\mtx@k) {\footnotesize\expandafter\mtx@split@gol\mtx@tmp\@nil};
            }
            \only<\gol>{
                \node[MTX@BLK=@secondary!70!white] (\x-\y) at (\x*\mtx@k, -\y*\mtx@k) {};
            }
            \only<\ded>{
...
\endscope
\draw[rounded corners=\p@,@primary,line width=1.65\p@] (.5*\mtx@k, -.5*\mtx@k) rectangle ++(#2*\mtx@k, -#3*\mtx@k);
\endscope}
