...
\begin{frame}[c]{Eigenschaften für die Analyse}
    ...
    \begin{block}<2->{Termination}
        \onslide<3->{Ein Algorithmus terminiert, wenn er nach endlich vielen Schritten zum Ende kommt.}
    \end{block}\bigskip
    \begin{block}<6->{Partielle Korrektheit}
        \onslide<7->{Wenn der Algorithmus für eine Eingabe terminiert, ist die erzeugte Ausgabe korrekt.}
    \end{block}
    }{%
...
\begin{frame}[c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Algorithmus beschreiben \Time{2}]
        ...
        \textit{Hinweis: Auf den Elementen muss keine \say{numerische} Ordnung definiert sein (Sie können aber vergleichen, ob ein Element größer als ein anderes ist).}
    \end{exercise}
\end{frame}
