...
\onslide<7->{\parbox[5em]{.29\linewidth}{\SetAlgoVlined\footnotesize
\begin{algorithm}[H]
...
\begin{frame}{Schema eines Algorithmus}
    \pause\begin{center}
        \begin{tikzpicture}
            ...
            \node (V) at(-5,0) {Vorbedingungen};
            \node (N) at(5,0) {Nachbedingungen};
    ...
    \begin{itemize}[<+(1)->]
        ...
        \item Ein Algorithmus benötigt (formal): \begin{itemize}
            \item Vorbedingungen \textcolor{gray}{(wie \say{positive ganze Zahl})}
            \item Nachbedingungen \textcolor{gray}{(wie \say{negative Fließkommazahl})}
        \end{itemize}
