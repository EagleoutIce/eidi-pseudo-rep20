...
\begin{tikzpicture}[scale=1.1, every node/.style={transform shape}, rec/.style={block,minimum width=3.25cm},irec/.style={iblock,minimum width=3.25cm}]
    ...
    \begin{scope}[every path/.style={link,rounded corners=1.25pt}]
        ...
        \onslide<7->{\draw (link) -- ++(2,0) node[right, irec] (prog) {Program};}
...
\end{plainjava}
        \pause{}speichert Java für \bjava{rnd} die Referenz,\pause{} an der sich die eigentlichen Daten des Objekts befinden. \textcolor{gray}{Stichwort: Heap und Stack.}
...
\begin{frame}{Strings}
    ...
    \begin{itemize}[<+(1)->]
        \widei
        \item Jede Variable\thinspace /\thinspace jedes Objekt kann in Java mittels \bjava{toString()} in einen \bjava{String} konvertiert werden.
