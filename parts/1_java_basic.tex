...
\begin{frame}{Konvertierung primitiver Datentypen}
    \begin{itemize}[<+(1)->]
        ...
        \item Unter der Einschränkung des Wertebereichs kann durch \bjava{(:lan:Datentyp:ran:):lan:Ausdruck:ran:} eine explizite Konvertierung auch in umgekehrte Richtung erfolgen.\par
...
\begin{frame}[c,fragile]{Lösung}
    \begin{solve}<2->[Implizite Typkonvertierung]
        \begin{itemize}[<+(1)->]
            \item[a)] Implizit: \bjava{int}, \bjava{long}, \bjava{float} und \bjava{double}.
            \item[b)] Explizit zusätzlich: \bjava{byte} und \bjava{char}.
        \end{itemize}
    \end{solve}
\end{frame}
\fi
