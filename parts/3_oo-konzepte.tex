...
\begin{frame}[c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Kamera \Time{5}]
        \onslide<3->{Schreiben Sie eine \bjava{Camera} Klasse, \onslide<4->{mit drei Fließkommazahlen für die aktuelle Position (\(x,y,z\)). \onslide<5->{Von der Klasse soll es nicht möglich sein mehr als eine Instanz zu erzeugen.\medskip\par \onslide<6->{Weitere \say{Anfragen} sollen dasselbe Objekt zurückliefern.}}}}
    \end{exercise}
\end{frame}
