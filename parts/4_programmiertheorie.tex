...
\begin{frame}{Rekursive und iterative Programmierung}
    ...
    \begin{itemize}[<+(1)->]
        \widei
        \item Eine rekursive Methode lässt sich in zwei Komponenten gliedern: \begin{description}[Abbruchbedingung]
            \item[Abbruchbedingung] Das lösbare Teilproblem. Ende der Rekursion.\pause{} Hier wird die Methode nicht weiter referenziert.
            \item[Rekursiver Zweig] Enthält die rekursiv auszuführende Prozedur.
        \end{description}
    \end{itemize}
\end{frame}
...
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
    Approximieren Sie \(\mathbb{N}\) durch den Java Datentyp \bjava{long}. Sie dürfen \emph{keine} bestehenden Java Funktionen (wie beispielsweise \bjava{Math::abs}) benutzen.\medskip\par
...
\begin{frame}{Komplexitätsbetrachtung}
    ...
    \begin{definition}<6->[Effizienz]
        \pause{}Die Effizienz eines Programms wird durch dessen \emph{Speicher-}, sowie \emph{Laufzeitaufwand} bestimmt.
    \end{definition}
    \begin{itemize}[<+(1)->]
        \widei
        \item<8-> Letztere werden wir ausführlicher betrachten. Genauer: In welcher Komplexitätsklasse liegt der Algorithmus?
    \end{itemize}
\end{frame}
...
\end{plainjava}
    \pause{}\textit{Hinweis:} um eine Zahl in einen String zu konvertieren, kann Konkatenation,\pause{} sowie \bjava{Integer.toString(int)} verwendet werden.
    \end{exercise}\onslide<1->
\end{frame}
