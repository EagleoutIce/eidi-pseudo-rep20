...
\begin{frame}[fragile,c]{Rekursiver Konverter}
    ...
    \begin{exercise}<2->[Rekursion zur Konvertierung \Time{4}]
        \pause{}Schreiben Sie eine rekursive Java-Methode \bjava{int2bin(int)}, die einen positiven Integer in dessen Binärdarstellung (als \bjava{String}) verwandelt. Beispiel:\pause{}
\begin{plainjava}
...
\begin{frame}[fragile,c]{Numerisches Palindrom}
    ...
    \begin{exercise}<2->[Numerisches Palindrom \Time{5}]
        \pause{}Schreiben Sie eine rekursive Java-Methode \bjava{boolean check(int n)}, die einen positiven Integer \bjava{n} erhält und entscheidet, ob es sich um ein Palindrom handelt.\pause{}
        ...
        Sie dürfen Hilfsmethoden und die Funktionen der \bjava{Math}-Klasse verwenden, die Konvertierung der Zahl in einen String oder ein Array ist Ihnen allerdings nicht gestattet.
    \end{exercise}\onslide<1->
\end{frame}

\begin{frame}[fragile,c]{Numerisches Palindrom -- Lösung}
    \begin{solve}<2->[Numerisches Palindrom]
        \onslide<3->{Wir können die Zahl rekursiv umdrehen und mit der ursprünglichen Zahl vergleichen:}
\begin{plainjava}
!*\onslide<6->*!int reverseNumber(int number, int digit, int current) {
!*\onslide<7->*!    if (number == 0) return current;
!*\onslide<8->*!    return reverseNumber(!*\onslide<9->*!number / 10,!*\onslide<10->*! digit + 1,
!*\onslide<8->*!                !*\onslide<11->*!current * 10 + number % 10!*\onslide<8->*!);
!*\onslide<6->*!}
!*\onslide<4->*!
!*\onslide<4->*!boolean check(int n) {
!*\onslide<5->*!    return n == reverseNumber(n, 0, 0);
!*\onslide<4->*!}
\end{plainjava}
    \end{solve}
\end{frame}
...
\begin{frame}[fragile,c]{Numerisches Palindrom -- Lösung}
    ...
    \begin{solve}<1->[Numerisches Palindrom\hfill{}(Fortsetzung)]
        \lstfs{10}\onslide<2->{Alternativ, kann man mit \bjava{Math.log10(double)} die Stellenanzahl der Zahl herausfinden und so immer die erste und letzte Zahl abschneiden (analog zu \bjava{String::substring}). Die Variablen sind hier zur Übersicht:}
\begin{plainjava}
...
!*\onslide<7->*!    int first = n / largest; // Liefert größte Ziffer
!*\onslide<7->*!    int last = n % 10; // Liefert kleinste Ziffer
!*\onslide<8->*!    if(first != last)
!*\onslide<8->*!        return false;
!*\onslide<9->*!    return check((n % largest) / 10); // Schneidet größte & kleinste ab
!*\onslide<3->*!}
\end{plainjava}
    \end{solve}
\end{frame}
