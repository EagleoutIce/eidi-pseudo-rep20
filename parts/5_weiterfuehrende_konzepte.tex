...
\begin{frame}{Suchverfahren formal}
    \begin{itemize}[<+(1)->]
        \widei
        \item Suchverfahren haben die Aufgabe in einer Folge an Elementen,\pause{} die Elemente zu finden, welche einem Muster oder gewissen Eigenschaften entsprechen.
        \item Es gibt zwei Varianten: \begin{description}[Heuristisch]
            \item[Einfach] Durchlaufen Suchraum auf Basis einer Datenstruktur.
            \item[Heuristisch] Besitzen zusätzliches Wissen (Verteilung,~\ldots) über die Daten, die sie zur Beschleunigung der Suche verwenden.
        \end{description}
        \item Die zu suchenden \emph{Eigenschaften}, nennt man auch \emph{Merkmale}.
    \end{itemize}
\end{frame}
...
\begin{frame}[c]{Lineare Suche}
    ...
    \begin{exercise}<2->[Lineare Suche \Time{4}]
        \pause{}Schreiben Sie eine Methode \bjava{int linearSearch(int[] arr, int key)}, die ein Array mit der linearen Suche nach einem gegebene Schlüssel durchsucht und den (ersten) Index des Schlüssels zurückliefert.
...
\begin{plainjava}
!*\onslide<3->*!int linearSearch(int[] arr, int key){
!*\onslide<4->*!    for(int i = 0; i < arr.length; i++)
!*\onslide<5->*!        if(arr[i] == key)
!*\onslide<5->*!            return i;
!*\onslide<6->*!    return -1; // Magic Number für "nicht gefunden"
!*\onslide<3->*!}
\end{plainjava}
    \end{solve}
\end{frame}
...
!*\onslide<5->*!    return -1; // Magic Number für "nicht gefunden"
!*\onslide<2->*!}
\end{plainjava}
\end{frame}
\fi
...
\begin{frame}{Klassische Suchverfahren}
    \begin{itemize}[<+(1)->]
        \item[]\begin{description}[Linear]
            \item[Binär] \hypertarget<1>{mrk:binsearch}{}Durchsucht eine (z.B. aufsteigend) \emph{sortierte} Folge baumartig.\smallskip\par\pause{} Vergleicht das mittlere Element des aktuellen Suchbereiches mit dem Suchbegriff.\pause{} Ist er größer, wird rechts, ist er kleiner, wird links von der Mitte weiter gesucht.\pause{} Die Komplexität beträgt \(\O(\log n)\) (ohne Sortieren), da jeder Vergleich den restlichen Suchbereich halbiert.
        \end{description}
        \vspace*{1em}
\begin{center}
    \begin{minipage}[c]{.35\linewidth}
        \centering\onslide<7->{\scalebox{.65}{\begin{forest}
    ...
    \end{minipage}\qquad\begin{minipage}[c]{.35\linewidth}
        \onslide<8->{\scalebox{.65}{\begin{tikzpicture}
...
\begin{frame}[c]{Binäre Suche}
    ...
    \begin{exercise}<2->[Binäre Suche \Time{6}]
        \pause{}Schreiben Sie eine Methode \bjava{int binarySearch(int[] arr, int key)}, die ein bereits aufsteigend sortiertes Array mit der binären Suche nach einem gegebene Schlüssel durchsucht und den Index des Schlüssels zurückliefert.
        Wird der Schlüssel nicht gefunden, so soll die Methode einen beliebigen negativen Integer zurückliefern.
    \end{exercise}\onslide<1->
\end{frame}
...
!*\onslide<4->*!    }
!*\onslide<9->*!    return -1; // Magic Number für "nicht gefunden"
!*\onslide<2->*!}
\end{plainjava}
\end{frame}
\fi
...
\begin{frame}{Sortierverfahren formal}
    \begin{itemize}[<+(1)->]
        \item Ein Prozess, bei dem Elemente auf Basis eines Ordnungskriteriums in eine Reihenfolge gebracht werden.
        \item Daten werden sortiert um beispielsweise Suchen zu vereinfachen.
        \item Wir unterscheiden zwei Arten: \begin{description}[Extern]
            \item[Intern] Hier liegt der gesamte Datenbestand im Arbeitsspeicher,\pause{} alle Elemente sind zugreifbar (Array,~\ldots)
            \item[Extern] Der Datenbestand ist (überwiegend) ausgelagert.\pause{} Für das Sortieren werden meist nur die obersten Element eines Stapels betrachtet (Dateien,~\ldots).\pause{} Oft erlauben Hilfsmethoden die selben Verfahren wie bei der internen Variante.
        \end{description}
    \end{itemize}
\end{frame}
...
\begin{multicols}{2}
    \begin{enumerate}
        \item<8-> \begin{tikzpicture}[scale=.65,every node/.style={transform shape},baseline=-.6ex]
            \List{4,2,1,3}{0}
            \draw[Kite-Kite] (1.north) to[bend left] (3.north);
        \end{tikzpicture}
        \item<9-> \begin{tikzpicture}[scale=.65,every node/.style={transform shape},baseline=-.6ex]
            \List{1,2,4,3}{1}
            \draw[Kite-Kite] (2) to[loop above] (2);
        \end{tikzpicture}
        \item<10-> \begin{tikzpicture}[scale=.65,every node/.style={transform shape},baseline=-.6ex]
            \List{1,2,4,3}{2}
            \draw[Kite-Kite] (3.north) to[bend left] (4.north);
        \end{tikzpicture}
        \item<11-> \begin{tikzpicture}[scale=.65,every node/.style={transform shape},baseline=-.6ex]
            \List{1,2,3,4}{4}
        \end{tikzpicture}
    \end{enumerate}
\end{multicols}
...
\begin{frame}[c]{Selectionsort implementieren}
    ...
    \begin{exercise}<2->[Selectionsort implementieren \Time{6}]
        \pause{}Gegeben sei eine Methode \bjava{int getMin(int[] arr, int i, int j)}, die korrekt das Minimum im Teilarray \([i, j]\) von \bjava{arr} findet. Weiter gegeben sei eine Methode \bjava{void swap(int[] arr, int i, int j)}, die die Elemente mit Index \(i\) und \(j\) in \bjava{arr} vertauscht.\medskip\par\pause Schreiben Sie eine Methode \bjava{int[] selectionSort(int[] arr)}, die das übergebene Array mittels Selectionsort als \emph{neues} aufsteigend sortiertes Array zurückliefert. Das übergebene Array darf nicht verändert werden.
    \end{exercise}\onslide<1->
\end{frame}
...
\begin{frame}{Sortierverfahren: Bubblesort}\alglabel{Bubblesort}
    \begin{definition}<2->[Bubblesort \hfill{}(iterativ)]
        \onslide<3->{Prinzip: Vertausche benachbarte Elemente wenn sie nicht in Sortierreihenfolge sind.}
    \end{definition}
    \begin{itemize}
...
\begin{minipage}{.9\linewidth}
    \begin{multicols}{2}
        \begin{enumerate}[<+(1)->]
            \item<7-> \begin{tikzpicture}[scale=.65,every node/.style={transform shape},baseline=-.6ex]
                \List{4,2,1,3}{5}
...
\begin{frame}[c]{Shakersort implementieren}
    ...
    \begin{exercise}<2->[Shakersort implementieren \Time{6}]
        \pause{}Gegeben sei eine Methode \bjava{void swap(int[] arr, int i, int j)}, die die Elemente mit Index \(i\) und \(j\) in \bjava{arr} vertauscht.\medskip\par\pause Schreiben Sie eine Methode \bjava{void shakerSort(int[] arr)}, die das übergebene Array mittels Shakersort aufsteigend sortiert. Dabei darf das Array \emph{nur} durch die übergeben \bjava{swap}-Methode verändert werden.\medskip\par
...
\begin{frame}[c]{Shakersort visualisieren}
    ...
    \begin{exercise}<2->[Shakersort visualisieren \Time{5}]
        ...
        \onslide<5->{\begin{center}
            \begin{tikzpicture}[scale=.65,every node/.style={transform shape},baseline=-.6ex]
                \XList{2, 3, 5, 7, 8, 3, 9}
...
\begin{frame}[c]{Shakersort visualisieren -- Lösung}
    ...
    \begin{enumerate}[<+(1)->]
        \widei
        \setcounter{enumi}{3}
        \item \begin{tikzpicture}[gray, scale=.65,every node/.style={transform shape},baseline=-.6ex]
            \XList{-15, 0, 8, 8, 9, 13, 23}
            \draw[thick] ([yshift=1mm]3.north west) -- ([yshift=1mm]5.north east);
            \draw[Kite-Kite] (5.south) to[bend left] (4.south);
        ...
        \end{tikzpicture} (down)\medskip\\
        \pause \textit{Dieser Schritt kann theoretisch weggelassen werden, wenn man die optimierte Version von Shakersort benutzt. Diese bricht ab, sobald in einem Durchlauf nicht mehr getauscht wird.}
    \end{enumerate}
    \onslide<1->
    \end{solve}
\end{frame}
...
\savebox{#3}{\begin{tikzpicture}
    \foreach[count=\i,remember=\i as \li (initially 0)] \n in {#1}{
        \ifnum\i=#2
            \node[inner sep=4pt,iball,outer sep=0pt,scale=.8] (\i) at(.7*\i,0) {\n};
        \else
            \node[inner sep=4pt,ball,outer sep=0pt,scale=.8] (\i) at(.7*\i,0) {\n};
        \fi
...
\begin{frame}[c]{Mergesort Pseudocode}
    ...
    \begin{exercise}<2->[Mergesort Pseudocode \Time{7}]
        \onslide<3->{Formulieren Sie Pseudocode, welcher eine gegebene Liste \(l\) mit Elementen \(l_i \in \mathbb{R}\) und Länge \(n\) durch Mergesort aufsteigend sortiert.\medskip\par
        Sie dürfen weitere Routinen formulieren. Es sei eine korrekt implementierte Routine \T{cut(l, i, j)} mit folgendem Vertrag gegeben:}\RestyleAlgo{tworuled}\onslide<4->{
\begin{algorithm}[H]
    \PreCode
...
\end{algorithm}
        \medskip\par}
        \onslide<5->{Weiter dürfen Sie mit \(x_i\) direkt auf das Listenelement an Stelle \(i\) in \(x\) zugreifen (sofern \(1 \leq i \leq n_x\) mit Länge \(n_x\)). Der Ausdruck \say{\T{x + l\textsubscript{i}}} erzeugt eine \emph{neue} Liste \((x_1, \ldots, x_n, l_i)\). Ebenso \say{\T{x + l}} mit \((x_1, \ldots, x_n, l_1, \ldots, l_n)\).}
    \end{exercise}\onslide<1->
\end{frame}
...
\begin{frame}[c]{Mergesort Pseudocode -- Lösung}
    \begin{solve}<2->[Mergesort Pseudocode]
        ...
        \PreCode
            \onslide<4->{\KwIn{Liste \(l = (l_1, \ldots, l_n)\)}
            \KwOut{Aufsteigend sortierte Liste \(l\).}}
        \StartCode
            \onslide<5->{\Comment{Ein- oder Nullelementig, bereits sortiert:}
            \onslide<6->{\lIf{\(n \leq 1\)}{\KwRet{l}}}
            \(left := \text{\T{cut}}(l, 1, \lfloor\sfrac{n}{2}\rfloor)\)\;
            \(right := \text{\T{cut}}(l, \lfloor\sfrac{n}{2}\rfloor, n)\)\;}
            \onslide<7->{\Comment{Rekursion}
            \(left := \text{\T{mergesort}}(left)\)\;
            \(right := \text{\T{mergesort}}(right)\)\;}
            \onslide<8->{\KwRet \(\text{\T{merge}}(left, right)\)\Comment*[l]{Auf der nächsten Folie}}
            \caption{mergesort(t)}
    \end{algorithm}}
    \end{solve}
\end{frame}
...
\begin{frame}[c]{Mergesort Pseudocode -- Lösung}
    ...
    \begin{solve}<2->[Mergesort Pseudocode\hfill(Fortsetzung)]
        \onslide<3->{\small\begin{algorithm}[H]
        \PreCode
            \onslide<4->{\KwIn{Aufsteigend sortierte Listen \(left\) und \(right\) (Längen \(n_{left}\), \(n_{right}\))}
            \KwOut{Aufsteigend sortierte Liste aus \(left\) und \(right\)}}
        \StartCode
            \onslide<5->{\(res := ()\)\Comment*[l]{Initialisiere leere merge-Liste}}
            \onslide<6->{\While(\Comment*[h]{Beide Listen enthalten noch Elemente}){%
                $left \neq ()$ \KwAnd $right \neq ()$%
            }{%
                \onslide<7->{\eIf{$left_1 \leq right_1$}{
                    \onslide<8->{\(res := res + left_1\);
                    \(left = \text{\T{cut}}(left, 2, n_{left})\)\;}
                }{
                    \onslide<9->{\(res := res + right_1\);
                    \(right = \text{\T{cut}}(right, 2, n_{right})\)\;}
                }}
            }}
            \onslide<10->{\(res := (res + left) + right\)\Comment*[l]{Es gilt $left = () \lor right = ()$}}
            \onslide<11->{\KwRet{res}\;}
...
\begin{frame}[c]{Quicksort implementieren}
    ...
    \begin{exercise}<2->[Quicksort implementieren \Time{6}]
        \pause{}Gegeben sei eine Methode \bjava{void swap(int[] arr, int i, int j)}, die die Elemente mit Index \(i\) und \(j\) in \bjava{arr} vertauscht.\medskip\par\pause Schreiben Sie eine Methode \bjava{void quickSort(int[] arr)}, die das übergebene Array mittels Quicksort aufsteigend sortiert. Als Pivot-Element soll stets das erste Element gewählt werden.\medskip\par\pause{}\textit{Tipp:} Arbeiten Sie mit einer Hilfsmethode \bjava{void quickSort(int[] arr,} \bjava{int l, int r)}, welche das Quicksort-Verfahren auf den Array-Ausschnitt \([l, r]\) anwendet.
    \end{exercise}\onslide<1->
\end{frame}
...
\begin{frame}[fragile,c]{Quicksort implementieren -- Lösung}
    \addtocounter{solve}{-1}%
    \begin{solve}<1->[Quicksort implementieren\hfill(Fortsetzung)]
\lstfs{10}\begin{plainjava}
!*\onslide<2->*!static int partition(int[] arr, int l, int r) {
!*\onslide<3->*!    int end = r;
!*\onslide<4->*!    swap(arr, l, end); // Einfach: Pivotelement ans Ende tauschen
!*\onslide<5->*!    int pivot = arr[end];
!*\onslide<6->*!    l -= 1; // durch do wird l++ einmal ausgeführt: wir setzen l zurück
!*\onslide<6->*!    // ebenso, wird so zugesichert, dass r eins links von l ist.

!*\onslide<7->*!    while (l < r) {
!*\onslide<8->*!        do { l++; } while (arr[l] < pivot);
!*\onslide<9->*!        do { r--; } while (arr[r] >= pivot && l < r);
!*\onslide<10->*!        if (l < r) swap(arr, l, r);
!*\onslide<7->*!    }
!*\onslide<11->*!    swap(arr, l, end); // Tausche Pivot zu Marker
!*\onslide<12->*!    return l;
!*\onslide<2->*!}
    \end{plainjava}
    \end{solve}
...
\begin{frame}{Sortierverfahren: Heapsort}\alglabel{Heapsort}
    ...
    \vfill
    \begin{center}\onslide<7->{%
        \parbox{5.65em}{\centering\scalebox{.625}{\begin{forest}
            for tree={inner sep=4pt,ball,font=\sffamily,outer sep=1pt}
            [1,iball[3[4][,phantom]][2]]
        \end{forest}}} \(\to\) \parbox{5.65em}{\centering\scalebox{.625}{\begin{forest}
            for tree={inner sep=4pt,ball,font=\sffamily,outer sep=1pt}
            [2,iball[3][4]]
        \end{forest}}} \(\to\) \parbox{5.65em}{\centering\scalebox{.625}{\begin{forest}
            for tree={inner sep=4pt,ball,font=\sffamily,outer sep=1pt}
...
\begin{frame}{Exkurs: Heap}
    ...
    \begin{definition}<2->[Heap]
        \onslide<3->{Binärbaum, mit: \(\text{Eltern} \leq \text{Kinder}\) (\textit{Min-Heap}, für \textit{Max-Heap}: \(\geq\)).}
    \end{definition}
    \begin{itemize}
        \itemsep1.1pt
        \item<4-> Neue Elemente werden unten als neues Kind eingefügt (markiert durch~$\circ$). \onslide<5->{Verletzen diese die Heap-Eigenschaft, werden solange kleinere Kinder nach oben getauscht, bis sie wiederhergestellt ist.}
        \item<6-> Beim Entfernen eines Elements wird das \say{letzte Blatt} als neue Wurzel getauscht. Anschließend wird wieder getauscht\ldots
    ...
    \begin{center}\onslide<8->{%
        ...
        \end{forest}}} \(\overset{1}{\to}\) \parbox{5.65em}{\centering\scalebox{.625}{\begin{forest}
            for tree={inner sep=4pt,ball,font=\sffamily,outer sep=1pt}
            [4,name=4[1,name=1][,fill=white]]
            \draw[Kite-Kite] (4) to[bend right] (1);
        \end{forest}}} \(\overset{2}{\to}\) \parbox{5.65em}{\centering\scalebox{.625}{\begin{forest}
            for tree={inner sep=4pt,ball,font=\sffamily,outer sep=1pt}
            [1[4[,fill=white][,phantom]][2]]
        \end{forest}}} \(\overset{3}{\to}\) \parbox{5.65em}{\centering\scalebox{.625}{\begin{forest}
            for tree={inner sep=4pt,ball,font=\sffamily,outer sep=1pt}
            [1[4,name=4[3,name=3][,fill=white]][2]]
...
\centering\vspace*{2em}\par\begin{tikzpicture}
    ...
    \onslide<3->{\node[right=1cm,yshift=4.5mm,scale=1.125] at (current bounding box.east) {\fancyqr[link]{https://github.com/EagleoutIce/Episode-Heaps/tree/main}};}
\end{tikzpicture}
\end{frame}
...
\begin{frame}[c]{Sortierverfahren: Zusammenfassung}
    ...
    \midrule
        ...
        \linksort{Bubblesort}    & \(\O(n^2)\)\textcolor{gray}{, \(\O(n)\)}      & \(\O(n^2)\)      & \(\O(n^2)\)      & \(\O(1)\) & iterativ \\
    ...
    \scriptsize\textit{Dies folgt den Implementationen der Vorlesung. Bereits leichte Modifikationen (wie: prüfe zuerst ob die Liste bereits sortiert ist) können die Daten verändern (beispielsweise einen best-case von \(\O(n)\) im Falle von Bubblesort).}\vspace*{-3.75mm}
\end{frame}

\begin{frame}{Eigenschaft: Stabil}
    \begin{definition}<2->[Stabiles Sortierverfahren]
        \onslide<3->{Ein Sortierverfahren heißt stabil, wenn es die Reihenfolge (aus Sicht des Sortierschlüssels) gleicher Element nicht ändert.}
    \end{definition}
    \begin{itemize}
        \widei
        ...
        \onslide<6->{\begin{center}
            ...
            \end{tabular}\quad\(\Longrightarrow\)\quad\begin{tabular}{cc}
                ...
                27 & Hansine \\
                32 & Apfeltine
            \end{tabular}\qquad\parbox{7.25em}{\itshape\footnotesize Hans sicher\\vor Inga.}
        \end{center}}
        \item<7-> Stabile Sortierverfahren (nach Implementation der Vorlesung): \onslide<8->{\linksort{Insertionsort}, \linksort{Bubblesort} und \linksort{Mergesort}.}
    \end{itemize}
\end{frame}
...
\begin{frame}[c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Sortieralgorithmen abwägen \Time{2}]
        \linksort{Quicksort} hat eine schlechtere \emph{worst-case} Laufzeit als \linksort{Mergesort}, worin ist es besser als \linksort{Mergesort}?
    \end{exercise}
\end{frame}

\begin{frame}[c]{Lösung}
    \begin{solve}<2->[Sortieralgorithmen abwägen]
        \pause{}\linksort{Mergesort} benötigt einen zusätzlichen Speicherplatz von \(\O(n)\), \linksort{Quicksort} ist in-place implementiert und benötigt lediglich \(\O(\log n)\) Speicherplatz zum Verwalten der rekursiven Aufrufe. Weiter hängt alles an der Wahl des Pivotelements. Gute Wahlen des Pivotelements (Median der Mediane) erlauben es, die \emph{worst-case} Laufzeit auf \(\O(n \log n)\) zu reduzieren (ebenso, wie sich \linksort{Mergesort} In-Place implementieren lässt).
    \end{solve}
\end{frame}
...
\begin{frame}[c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Sortieralgorithmen zeichnen, I \Time{5}]
        \pause{}Veranschaulichen Sie den Sortierprozess der Zahlen \say{\(5,7,2,5,3,-19,6\)} unter der Verwendung von \linksort{Mergesort} grafisch. Geben Sie jeden Schritt an und sortieren Sie aufsteigend.
    \end{exercise}
\end{frame}
...
\begin{center}
    ...
    \end{forest}}\qquad~~\qquad\pause{}\scalebox{.75}{
        \begin{forest}
...
\begin{frame}[c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Sortieralgorithmen zeichnen, II \Time{5}]
        \onslide<3->{Veranschaulichen Sie den Sortierprozess der Zahlenfolge \(3, -5, -4, 2, 6\) unter der Verwendung von \linksort{Quicksort} grafisch. Nutzen Sie das in-place Verfahren durch Vertauschung aus der Vorlesung und sortieren Sie aufsteigend. \onslide<4->{Geben Sie den Zustand der Folge nach jeder Partitionierung aus und markieren Sie die Partitionen.}\medskip\par
...
\begin{frame}[c]{Lösung}
    \begin{solve}<2->[Sortieralgorithmen zeichnen, II]
        ...
        \begin{enumerate}
            ...
            \onslide<6->{\begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
                \QsList{6, -5, -4, 2, 3}{5}
                \node[below] at(1.south) {l};
                \node[below] at(4.south) {r};
                \draw[Kite-Kite] (1.north) to[bend left=10] (4.north);
            \end{tikzpicture}:\quad\begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
                \QsList{2, -5, -4, 6, 3}{5}
            ...
            \only<8->{\color{black}}\onslide<7->{Tausche \(l\) und Pivot:\quad\begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
                \QsList{2, -5, -4, 3, 6}{4}
            \end{tikzpicture}}
            \item<9-> \only<13->{\color{gray}}Links: \begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
                \QsList{2, -5, -4}{1}
...
\begin{frame}[c]{Lösung}
    ...
    \begin{solve}<2->[Sortieralgorithmen zeichnen, II\hfill(Fortsetzung)]
        \begin{enumerate}
            \setcounter{enumi}{2}
            \widei
            \item<3-> \only<7->{\color{gray}}Links: \begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
                \QsList{-4, -5}{1}
                ...
                \begin{scope}[xshift=3*3.5em,densely dashed,gray]
                    \QsList{3, 6}{0}
                \end{scope}
            \end{tikzpicture}\onslide<4->{\quad Tausch ans Ende:\quad\begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
                \QsList{-5, -4}{2}
                \begin{scope}[xshift=2*3.5em,densely dashed,gray]
                    \QsList{2}{0}
                \end{scope}
                \begin{scope}[xshift=3*3.5em,densely dashed,gray]
                    \QsList{3, 6}{0}
                \end{scope}
            \end{tikzpicture}}\medskip\par\onslide<5->{\begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
                \QsList{-5, -4}{2}
                ...
                \end{scope}
            \end{tikzpicture}}\medskip\par
            \only<7->{\color{black}}\onslide<6->{Tausche \(l\) und Pivot:\quad\begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
                \QsList{-5, -4}{2}
                \begin{scope}[xshift=2*3.5em,densely dashed,gray]
                    \QsList{2}{0}
                \end{scope}
                ...
                \end{scope}
            \end{tikzpicture}}
            \item<8-> Rechts in\quad\begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
                \QsList{-4, -5, 2}{3}
                \begin{scope}[xshift=3*3.5em,densely dashed,gray]
                    \QsList{3, 6}{0}
                \end{scope}
            \end{tikzpicture}\quad ist leer. \onslide<9->{Vertauschungen ändern nichts.}
            \item<10-> Rechts in\quad\begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
                \QsList{2, -5, -4, 3, 6}{4}
            \end{tikzpicture}\quad besteht mit \begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
                \QsList{6}{0}
            \end{tikzpicture} nur aus einem Element. \onslide<11->{Vertauschungen ändern nichts}.
        \end{enumerate}
        \onslide<12->{Finales Ergebnis:\quad\begin{tikzpicture}[scale=.6,every node/.style={transform shape},baseline=-.6ex]
            \QsList{-5, -4, 2, 3, 6}{0}
        \end{tikzpicture}}
    \end{solve}
\end{frame}
