...
\begin{frame}[c]{Binäre Suche}
    ...
    \begin{exercise}<2->[Binäre Suche \Time{6}]
        \pause{}Schreiben Sie eine Methode \bjava{int binarySearch(int[], int)}, die ein bereits aufsteigend sortiertes Array mit der binarySearch Suche nach einem gegebene Schlüssel durchsucht und den Index des Schlüssels zurückliefert.
        Wird der Schlüssel nicht gefunden, so soll die Methode einen negativen Wert zurückliefern.
    \end{exercise}\onslide<1->
\end{frame}
...
!*\onslide<7->*!    for(int i = 0; i < arr.length - 1; i++) {
!*\onslide<8->*!        int min = getMin(arr, i, arr.length);
!*\onslide<9->*!        swap(arr, i, min);
!*\onslide<7->*!    }
!*\onslide<7->*!
...
\lstfs{10}\begin{plainjava}
!*\onslide<3->*!public static void shakerSort(int[] arr) {
...
\begin{frame}[c]{Mergesort Pseudocode}
    ...
    \begin{exercise}<2->[Mergesort Pseudocode \Time{6}]
        \pause{}Formulieren Sie Pseudocode, welcher ein gegebene Liste \(l\) mit Elementen \(l_i \in \mathbb{R}\) und Länge \(n\) durch den Mergesort Algorithmus aufsteigend sortiert.\pause{}\medskip\par
...
\begin{algorithm}[H]
    \PreCode
        \KwIn{Liste \(t = (t_1, \ldots, t_n)\), \(i, j \in \{1, \ldots, n\}\)}
        \KwOut{Teilliste \((t_i, \ldots, t_j)\), falls \(i \leq j\). Sonst (wenn \(i > j\)) eine leere Liste.}
    \StartCode
\end{algorithm}
        \medskip\par
...
\begin{frame}{Exkurs: Heap}
    ...
    \end{definition}
    \begin{itemize}
        \item<4-> Neue Elemente werden von Unten an als neues Kind hinzugefügt. \onslide<5->{Wird die Eigenschaft verletzt, werden solange kleinere Kinder nach oben getauscht bis die Eigenschaft wiederhergestellt ist.}
...
\begin{frame}{Eigenschaft: Stabil}
    ...
    \end{definition}
    \begin{itemize}
        \item<4-> Beispiel: Eine bereits alphabetisch nach Nachnamen sortierte Liste an Personen wird nun nach Alter sortiert. \onslide<5->{Bei einem stabilen Sortierverfahren bleibt die alphabetische Sortierung bei Personen des selben Alters erhalten.}
...
\begin{frame}[c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Sortieralgorithmen zeichnen, I \Time{5}]
        \pause{}Veranschaulichen Sie den Sortierprozess der Zahlenfolge \(5,7,2,5,3,-19,6\) unter der Verwendung von \emph{Mergesort} grafisch. Geben Sie jeden Schritt an und sortieren Sie aufsteigend.
    \end{exercise}
\end{frame}
...
\begin{frame}[c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Sortieralgorithmen zeichnen, II \Time{5}]
        \pause{}Veranschaulichen Sie den Sortierprozess der Zahlenfolge \(3, -5, -4, 2, 6\) unter der Verwendung von \emph{Quicksort} grafisch. Nutzen Sie das in-place Verfahren durch Vertauschung aus der Vorlesung und sortieren Sie aufsteigend.\pause{} Geben Sie den Zustand der Folge nach jeder Partitionierung aus und markieren Sie die Partitionen.
    \end{exercise}
\end{frame}
...
\begin{frame}[c]{Lösung}
    \begin{solve}<2->[Sortieralgorithmen zeichnen, II]
        \onslide<3->{Ich führe hier zusätzlich auf, das mit den \bjava{l}- und \bjava{r}-Markern bei der Partitionierung geschieht.}
        \begin{enumerate}
            \item<4-> \onslide<8->{\color{gray}}\begin{tikzpicture}[scale=0.6,gray,every node/.style={transform shape},baseline=-0.6ex]
                \QsList{3, -5, -4, 2, 6}{1}
            \end{tikzpicture}\onslide<5->{\quad Tausch ans Ende:\quad\begin{tikzpicture}[scale=0.6,gray,every node/.style={transform shape},baseline=-0.6ex]
                \QsList{6, -5, -4, 2, 3}{5}
            \end{tikzpicture}\medskip\par
            \onslide<6->{\begin{tikzpicture}[scale=0.6,gray,every node/.style={transform shape},baseline=-0.6ex]
                \QsList{6, -5, -4, 2, 3}{5}
                \node[below] at(1.south) {l};
                \node[below] at(4.south) {r};
                \draw[Kite-Kite] (1.north) to[bend left=10] (4.north);
            \end{tikzpicture}:\quad\begin{tikzpicture}[scale=0.6,gray,every node/.style={transform shape},baseline=-0.6ex]
                \QsList{2, -5, -4, 6, 3}{5}
                \node[below] at(4.south) {l,r};
            \end{tikzpicture}}}\medskip\par
            \onslide<8->{\color{black}}\onslide<7->{Tausche \(l\) und Pivot:\quad\begin{tikzpicture}[scale=0.6,every node/.style={transform shape},baseline=-0.6ex]
                \QsList{2, -5, -4, 3, 6}{4}
            \end{tikzpicture}}
        \end{enumerate}
    \end{solve}
