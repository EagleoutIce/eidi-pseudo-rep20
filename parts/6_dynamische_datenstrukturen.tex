...
\begin{frame}[fragile]{Listen -- Einfach verkettete Liste}
    ...
    \begin{itemize}[<+(1)->]
        \widei
        \item Jedes Element besitzt als Objekt eine Referenz auf das nächste.
        \item Das erste Element bezeichnen wir als \emph{Head}. Das Letzte, welches auf einen definierten letzten Wert (wie \bjava{null}) zeigt, nennen wir \emph{Tail} oder \emph{Foot}.
    \end{itemize}\pause%
...
\centering\begin{tikzpicture}[scale=.7,every node/.style={transform shape}]
...
\begin{frame}{Listen -- Einfach verkettete Liste, II}
    \begin{itemize}[<+(1)->]
        \item Vorteil einer einfach verketten Liste:\pause{} Das Löschen und Hinzufügen neuer Elemente ist leicht (\(\O(1)\)).\pause{} Nachteil: Der direkte Zugriff ist nur durch ein Traversieren möglich.
...
\begin{enumerate}[<+(1)->]
    \widei
    \item \begin{tikzpicture}[scale=.75,every node/.style={transform shape}]
            \node[enode] (a) at (0,0) {\Element{4}};
            \node[enode] (b) at (2.25,0) {\Element{2}};
    ...
    \item \begin{tikzpicture}[scale=.75,every node/.style={transform shape}]
        ...
        \draw[-Kite] ($(b.east)+(-1em,0)$) -- ++(.5,1) -- (e.west);
        \fill ($(b.east)+(-1em,0)$) circle (2pt);
...
\end{plainjava}
        \pause{}Schreiben Sie eine Methode \bjava{Element appendLast(Element root, int i)}, die ein neues \bjava{Element} mit Wert \(i\) an das Ende der Liste, anhängt und diese Liste zurückliefert. Die Liste soll durch den Kopf \bjava{root} gekennzeichnet sein. Sie dürfen die ursprüngliche Liste verändern.
    \end{exercise}
\end{frame}

\begin{frame}[fragile,c]{Lösung}
    \begin{solve}<2->[Einfach verkettete Liste - Einfügen]
        Die erste Methode behandelt den Sonderfall \say{leere Liste}. Die Zweite traversiert die Liste so lange,\pause{} bis wir am \emph{Foot} angekommen sind und ersetzt dann den Verweis des vorletzten Elements:\pause{}
\begin{plainjava}
...
% NOTE: TODO: NOTE: gutes Beispiel: Undo & Redo, Verarbeitungsverlauf.
...
\begin{frame}[fragile]{Listen -- Doppelt verkettete Liste}
    ...
    \begin{itemize}[<+(1)->]
        ...
        \item Die doppelt verkettete Liste hält nun jeweils auch die Referenz auf das vorherige Element.
    \end{itemize}%
    \pause{}
...
\end{minipage}
    \begin{itemize}[<+(1)->]
        \widei
        \item Den \say{Datentyp} einfach verkettete Liste lagern wir in eine neue Klasse \bjava{LinkedList} aus, die auch die Operationen wie hinzufügen und löschen übernimmt.
    \end{itemize}%
\end{frame}
...
\begin{frame}{Datenstruktur: Stack}
    ...
    \begin{itemize}[<+(1)->]
        ...
        \item Die Verwaltung rekursiver Methoden erfolgt über eine Art Stack. So \say{überlagern} die Parameter im rekursiven Aufruf die alten, bis die Methode wieder verlassen wird.
    \end{itemize}
\end{frame}
...
\begin{frame}{Datenstruktur: Stack, II}
    \begin{itemize}[<+(1)->]
        \widei
        \item Stacks lassen sich als Listen implementieren,\pause{} die nur Zugriffe wie \bjava{appendLast(Element)} und \bjava{removeLast(Element)} zulassen.
        \item Auf Stacks kennzeichnen wir zwei wichtige Operationen: \begin{description}[push]
...
\begin{frame}{Datenstruktur: Queue}
    \hypertarget<1>{mrk:Queue}{}\begin{itemize}[<+(1)->]
        ...
        \item Es existieren Derivate, wie die \emph{double-ended queue} (ein- und ausreihen auf beiden Seiten) oder die \emph{priority queue} (Elemente mit höherer Priorität werden bei \T{enqueue} zuerst aus der Schlange genommen).
    \end{itemize}
\end{frame}

\begin{frame}{Datenstruktur: Queue, II}
    \begin{itemize}[<+(1)->]
        \widei
        \item Queues lassen sich als eine Liste implementieren,\pause{} die nur Zugriffe wie \bjava{prependFirst(Element)} und \bjava{removeLast(Element)} zulässt.
        \item Auf Queues kennzeichnen wir zwei wichtige Operationen: \begin{description}[dequeue]
...
\centering\begin{tikzpicture}[scale=.85,every node/.style={transform shape}]
    ...
    \draw[-Kite,rounded corners=2pt] (-.5,.25) node[left] {push} -| ++(1.75,-.75);
    \draw[Kite-,rounded corners=2pt] (3.5,.25) node[right] {pop} -| ++(-1.75,-.75);
\end{tikzpicture}
            \end{column}
            \pause{}%
            \begin{column}{.45\textwidth}
\centering\begin{tikzpicture}[scale=.85,every node/.style={transform shape}]
    ...
    \node at(1.5,-2.5) {42};
    \node at(1.5,-1.5) {13};
    \draw (0,-3) -- ++(3,0); \draw (0,-2) -- ++(3,0); \draw (0,-1) -- ++(3,0);
    \draw[Kite-] (3,-2.5) -- ++(1,0) node[right] {head};
    \draw[Kite-] (3,-1.5) -- ++(1,0) node[right] {tail};
    \draw[-Kite,rounded corners=2pt] (-.5,.25) node[left] {enqueue} -| ++(1.75,-.75);
    \draw[Kite-,overlay,rounded corners=2pt] (3.5,-4.25) node[right] {dequeue} -| ++(-1.75,.75);
\end{tikzpicture}
            \end{column}
        \end{columns}
    \end{center}
...
\begin{frame}{Bäume: Formal}
    \begin{itemize}[<+(1)->]
        ...
        \item Knoten erhalten verschiedene Bezeichner: \begin{description}[Innerer]
            \widei
            ...
            \item[Innerer] Ein innerer Knoten ist jeder Knoten, der weder Blatt noch Wurzel ist.\pause{} Ein innerer Knoten hat genau einen Elternknoten.
        \end{description}
    \end{itemize}
\end{frame}
...
\begin{frame}{Bäume: Mathematisch, II}
    \begin{itemize}[<+(1)->]
        \widei
...
\centering\scalebox{.625}{\begin{forest}
    for tree={inner sep=4pt,ball,font=\Large\sffamily,outer sep=1pt,edge=-Kite}
...
\end{plainjava}
    \end{minipage}\hfill{}\pause\begin{minipage}[c]{.55\linewidth}%
\centering\scalebox{.625}{\begin{forest}
    for tree={inner sep=4pt,ball,font=\Large\sffamily,outer sep=1pt,edge=Kite-Kite}
    [7[3[4[1][9]][,phantom]][6[1[,phantom][5]][3[,phantom][,phantom]]]]
\end{forest}}
\end{minipage}\vfill
    \begin{itemize}[<+(1)->]
...
\begin{frame}[c]{Traversierungsverfahren}
    \begin{center}\pause{}
    \begin{tikzpicture}[rounded corners=2pt, fakeblock/.style={rectangle,draw=@black},fakesource/.style={midway, centered,minimum height=\baselineskip,execute at begin node={\strut}}]
        \draw[fakeblock] (-1.5,0) rectangle ++ (3.5,.65) node[fakesource] {Breitendurchlauf};
    \draw[fakeblock] (4,0) rectangle ++ (3.5,.65) node[fakesource] {Tiefendurchlauf};
    \draw[fakeblock] (1.5,-1.5) rectangle ++ (2.5,.65) node[fakesource] {Pre-Order};
    \draw[fakeblock] (4.5,-1.5) rectangle ++ (2.5,.65) node[fakesource] {In-Order};
    \draw[fakeblock] (7.5,-1.5) rectangle ++ (2.5,.65) node[fakesource] {Post-Order};
    \draw[-Kite] (4+3.5/2,0) to[in=90,out=270] (2.75,-.85);
    \draw[-Kite] (4+3.5/2,0) to[in=90,out=270] (5.75,-.85);
    \draw[-Kite] (4+3.5/2,0) to[in=90,out=270] (8.75,-.85);
    \draw[fakeblock] (1.5+.25,-1.5) |- ++(2,-.5) -- ++(0,.5);
    \draw[fakeblock] (4.5+.25,-1.5) |- ++(2,-.5) -- ++(0,.5);
    \draw[fakeblock] (7.5+.25,-1.5) |- ++(2,-.5) -- ++(0,.5);

    \draw[fakeblock] (0+.75-1.5,0) |- ++(2,-.5) -- ++(0,.5);
    \node at(.1 + 1.65-1.5, -.25) {\tiny Ebenen};

    \node at(.1 + 1.65-1.5, .75) [above] { \begin{tikzpicture}[
        scale=.425, every node/.style={transform shape},
        ring/.style={
            ball,%
        ...
        \begin{scope}[every path/.style={densely dotted, thin,black!45}]
            \draw (one) -- (two); \draw (one) -- (tre);
            \draw (two) -- (six); \draw (two) -- (sev);
            \draw (tre) -- (for); \draw (tre) -- (fiv);
            \end{scope}
        \begin{scope}[every path/.style={densely dashed,-Kite}]
            \draw (one) -- (two); \draw (two) -- (tre);
            \draw (tre) -- (six); \draw (six) -- (sev);
            \draw[,scale=.5,transform shape] (sev) -- (fiv);
            \draw (fiv) -- (for);
        \end{scope}
    \end{tikzpicture}};
    ...
    \node at(1.6 + 1.15, -2.15) [below] { \begin{tikzpicture}[
        ...
        \begin{scope}[every path/.style={densely dashed}]
            \draw[-Kite] (one) -- (two); \draw[-Kite] (two) -- (six);
            \draw[-Kite] (six) to[bend left] (sev);
            \draw[-Kite] (sev) to[out=85,in = 185,looseness=1.15] (tre);
            \draw[-Kite] (tre) -- (fiv); \draw[-Kite] (fiv) -- (for);
        \end{scope}
        %%DRAW DEFAULTS DENSELY DOTTED IN GRAY TO KEEP STYLE
    \end{tikzpicture}};
    ...
    \node at(4.6 + 1.15, -2.15) [below] { \begin{tikzpicture}[
        ...
        \begin{scope}[every path/.style={densely dotted, thin,black!45}]
            ...
            \end{scope}
        \begin{scope}[every path/.style={densely dashed}]
            \draw[-Kite] (six) -- (two); \draw[-Kite] (two) -- (sev);
            \draw[-Kite] (sev) to[bend left] (one);
            \draw[-Kite] (one) to[bend left] (fiv);
            \draw[-Kite] (fiv) -- (tre); \draw[-Kite] (tre) -- (for);
        \end{scope}
    ...
    \node at(7.6 + 1.15, -2.15) [below] { \begin{tikzpicture}[
        ...
        \begin{scope}[every path/.style={densely dotted, thin,black!45}]
            ...
            \end{scope}
        \begin{scope}[every path/.style={densely dashed}]
            \draw[-Kite] (six) -- (sev); \draw[-Kite] (sev) -- (two);
            \draw[-Kite] (two) to[bend left] (fiv);
            \draw[-Kite] (fiv) -- (for); \draw[-Kite] (for) -- (tre);
            \draw[-Kite] (tre) -- (one);
        \end{scope}
    \end{tikzpicture}};
    \end{tikzpicture}
...
\begin{frame}{Traversierung: Level-Order}
    \begin{itemize}[<+(1)->]
        \item Die Knoten werden Ebene für Ebene von links nach rechts besucht.
        \item \say{Besucht} steht hier für alle möglichen Varianten der Bearbeitung.
        \item Es handelt sich um einen Breitendurchlauf (\textit{breadth-first}).
    \end{itemize}\medskip
    \pause{}\begin{center}
        \scalebox{.625}{\begin{forest}
            for tree={inner sep=4pt,ball,font=\Large\sffamily,outer sep=1pt,minimum width=1.15cm,edge=-Kite,s sep=3.5em-level*.65em}
            [1,name=1[2,name=2[4,name=4[8,name=8][9,name=9]][5,name=5[10,name=10][11,name=11]]][3,name=3[6,name=6[12,name=12][13,name=13]][7,name=7[14,name=14][15,name=15]]]]
            \onslide<6->{
            \pgfinterruptboundingbox
            \scope[every path/.append style={draw,line width=2.75pt,lightgray!65!white,-Kite}]
                \path (-9.25,0|-1) node[left,gray] {\textbf{0}} -- (1);
                \path (1) -- (1-|9.25,0);
                \path (-9.25,0|-2) node[left,gray] {\textbf{1}} -- (2);
                \path (2) -- (3);
                \path (3) -- (3-|9.25,0);
                \path (-9.25,0|-4) node[left,gray] {\textbf{2}} -- (4);
                \path (4) -- (5); \path (5) -- (6); \path (6) -- (7);
                \path (7) -- (7-|9.25,0);
                \path (-9.25,0|-8) node[left,gray] {\textbf{3}} -- (8);
                \path (8) -- (9); \path (9) -- (10); \path (10) -- (11);
                \path (11) -- (12); \path (12) -- (13); \path (13) -- (14);
                \path (14) -- (15);
            \endscope
...
\begin{frame}{Traversierung: Pre-Order}
    \begin{itemize}[<+(1)->]
        ...
        \item Mit der Pre-Order Strategie werden erst der aktuelle Knoten und dann rekursiv beide Teilbäume nach der gleichen Regel besucht.
    \end{itemize}\medskip
    \onslide<4->{\begin{center}
        \scalebox{.625}{\begin{forest}
            for tree={inner sep=4pt,ball,font=\Large\sffamily,outer sep=1pt,minimum width=1.15cm,edge=-Kite,s sep=3.5em-level*.65em}
            [1,name=1[2,name=2[3,name=3[4,name=4][5,name=5]][6,name=6[7,name=7][8,name=8]]][9,name=9[10,name=10[11,name=11][12,name=12]][13,name=13[14,name=14][15,name=15]]]]
        \end{forest}}
    \end{center}}
\end{frame}
...
\begin{frame}{Traversierung: In-Order}
    \begin{itemize}[<+(1)->]
        \item Es handelt sich um einen Tiefendurchlauf (\textit{depth-first}).
        \item Mit dieser Strategie wird erst rekursiv ein Teilbaum bearbeitet, dann der Knoten selbst und dann rekursiv der andere Teilbaum.
    \end{itemize}\medskip
    \onslide<4->{\begin{center}
        \scalebox{.625}{\begin{forest}
            for tree={inner sep=4pt,ball,font=\Large\sffamily,outer sep=1pt,minimum width=1.15cm,edge=-Kite,s sep=3.5em-level*.65em}
            [8,name=8[4,name=4[2,name=2[1,name=1][3,name=3]][6,name=6[5,name=5][7,name=7]]][12,name=12[10,name=10[9,name=9][11,name=11]][14,name=14[13,name=13][15,name=15]]]]
        \end{forest}}}
    \end{center}
\end{frame}
...
\begin{frame}{Traversierung: Post-Order}
    \begin{itemize}[<+(1)->]
        \item Es handelt sich um einen Tiefendurchlauf (\textit{depth-first}).
        \item Mit der Post-Order Strategie werden erst rekursiv beide Teilbäume und dann der Knoten selbst bearbeitet.
    \end{itemize}\medskip
    \onslide<4->{\begin{center}
        \scalebox{.625}{\begin{forest}
            for tree={inner sep=4pt,ball,font=\Large\sffamily,outer sep=1pt,minimum width=1.15cm,edge=-Kite,s sep=3.5em-level*.65em}
...
\centering\vspace*{2em}\par\begin{tikzpicture}
    ...
    \onslide<3->{\node[below=2mm] at (current bounding box.south) {Mehr zu \say{Traversierungen} per Klick\ldots};}
    \onslide<3->{\node[right=1cm,yshift=4.5mm,scale=1.125] at (current bounding box.east) {\fancyqr[link]{https://github.com/EagleoutIce/Episode-Traversierung/tree/main}};}
\end{tikzpicture}
\end{frame}
...
\begin{frame}{Suchbäume}
    \begin{itemize}[<+(1)->]
        \widei
        \item Bäume können auch zur Sortierung nützliche Datenstrukturen sein.
        \item So existieren Suchbäume. \textcolor{gray}{(Alle Elemente im linken Teilbaum sind kleiner oder gleich der Wurzel, alle rechten größer \(\Rightarrow\) \hyperlink{mrk:binsearch}{binäre Suche}.)}
...
\begin{frame}{Arithmetische Bäume}
    ...
    \begin{center}
        \scalebox{.625}{\begin{forest}
            for tree={inner sep=4pt,ball,font=\Large\sffamily,outer sep=1pt,minimum width=1.15cm,edge=-Kite}
            [$*$[$+$[A][B]][$-$[C][D]]]
        \end{forest}}
    \end{center}
    \begin{itemize}[<+(1)->]
    \item Die Traversierungsverfahren entsprechen Notationsschemata: \begin{description}[Post-Order]
        \item[In-Order] repräsentiert die geläufige Infix-Notation: \((A + B) * (C - D)\). \textcolor{gray}{(Die Klammern entsprechen der Ausführreihenfolge und sind kein Teil.)}
...
\begin{frame}{Graphen}
    ...
    \end{definition}
    \begin{itemize}[<+(1)->]
        \item Die Graphdefinition gleicht der aus \say{Formale Grundlagen}. Sie wird hier zusammengefasst.
    \end{itemize}
    \pause{}
...
\def\basegraph{%
\scope[every node/.append style={ball,gray}]
    \node (0) at (0,0) {};
    \node (1) at (1,1.125) {};
    \node (2) at (1,-1.125) {};
    \node (3) at (2.5,1.125) {};
...
\begin{frame}{Begriffe in einem Graphen}
    ...
    \begin{description}[Zusammenhang]
        \widei
        \item<2->[Zusammenhang] Wenn jeder Knoten von jedem anderen aus (über die Kanten) erreicht werden kann.\pause{}\medskip % Gerichteter Graph: Unterscheidung zw. stark und schwach.
\begin{center}
    \hfill\begin{tikzpicture}[scale=.5,gray]
        \basegraph
        \onslide<3->{
            \draw[thick] (0) -- (1) -- (2) -- (3) -- (4) -- (5);
        }
        \node[above left] at(current bounding box.south east) {\faCheck};
    \end{tikzpicture}\pause\hfill\begin{tikzpicture}[scale=.5,gray]
        \basegraph
        \onslide<4->{
        ...
        \node[above left] at(current bounding box.south east) {\faCheck};
    \end{tikzpicture}\pause\hfill
    \begin{tikzpicture}[scale=.5,gray]
        \basegraph
        \onslide<5->{
            \draw[thick] (1) -- (0) -- (2);
            \draw[thick] (3) -- (5) -- (4);
        }
        \node[above left] at(current bounding box.south east) {\faClose};
    \end{tikzpicture}\hfill\null
\end{center}\pause{}
        \item<6->[Grad] Die Anzahl an Knoten, die mit einem Knoten verbunden sind. \onslide<7->{\textcolor{gray}{Ist der Graph gerichtet, so wird zwischen Eingangs- und Ausgangsgrad unterschieden.}}\medskip
\begin{center}
        \hfill\onslide<8->{\begin{tikzpicture}[scale=.5,gray]
        \basegraph
            \draw[thick] (0) -- (1) -- (2) -- (3) -- (4) -- (5) -- (3);
            \begin{pgfinterruptboundingbox}
            \begin{scope}[every node/.append style={font=\scriptsize\sffamily}]
                \node[left=1mm] at (0) {1};
                \node[left=1mm] at (1) {2};
                \node[left=1mm] at (2) {2};
                \node[right=1mm] at (3) {3};
                \node[right=1mm] at (4) {2};
                \node[right=1mm] at (5) {2};
            \end{scope}
            \end{pgfinterruptboundingbox}
    \end{tikzpicture}}\pause\hfill\onslide<9->{\begin{tikzpicture}[scale=.5,gray]
        \basegraph
        \pgfonlayer{background}
        \foreach \i in {0, ..., 5} {
            \foreach \j in {\i, ..., 5} {
        ...
        \begin{scope}[every node/.append style={font=\scriptsize\sffamily}]
            ...
            \node[left=1mm] at (1) {5};
            \node[left=1mm] at (2) {5};
            \node[right=1mm] at (3) {5};
            \node[right=1mm] at (4) {5};
            \node[right=1mm] at (5) {5};
        \end{scope}
        \end{pgfinterruptboundingbox}
    \end{tikzpicture}}\pause\hfill%
    \onslide<10->{\begin{tikzpicture}[scale=.5,gray]
        \basegraph
            \draw[thick,Kite-] (1) -- (0);
            \draw[thick,-Kite] (0) -- (2);
            \draw[thick,-Kite] (3) -- (5);
            ...
            \begin{pgfinterruptboundingbox}
            \begin{scope}[every node/.append style={font=\scriptsize\sffamily}]
                \node[left=1mm] at (0) {0/2};
                \node[left=1mm] at (1) {1/0};
                \node[left=1mm] at (2) {1/0};
                \node[right=1mm] at (3) {0/1};
                \node[right=1mm] at (4) {0/1};
                \node[right=1mm] at (5) {2/0};
                \node at (current bounding box.center) {Ein/Aus};
            \end{scope}
            \end{pgfinterruptboundingbox}
    \end{tikzpicture}}\hfill\null
\end{center}
...
\begin{frame}{Begriffe in einem Graphen, II}
    \pause{}
    \begin{description}[Pfad]
        \widei
        \item[Weg] Eine endliche Folge an Knoten, die durch Kanten (egal welcher Richtung), verbunden sind.\par\medskip\pause{}
\begin{center}
\onslide<3->{\begin{tikzpicture}[scale=.5,gray]
    ...
    \node[ball,gray] (6) at (5.25,0) {};
    \draw (0) -- (1) -- (3) -- (5) -- (1) -- (2) -- (5) (0) -- (2) -- (4) -- (5) -- (6);
    \onslide<4->{
        \foreach \i in {0,1,5,6} {\fill[@primary] (\i) circle [radius=4pt];}
        \node[above left] at(current bounding box.south east) {\faCheck};
    }
\end{tikzpicture}}\qquad\onslide<5->{\begin{tikzpicture}[scale=.5,gray]
    \node[ball,gray] (0) at (0,0) {};
    \node[ball,gray] (1) at (1,1.125) {};
    \node[ball,gray] (2) at (1,-1.125) {};
    \node[ball,gray] (3) at (2.5,1.125) {};
    ...
    \node[ball,gray] (5) at (3.75,0) {};
    \node[ball,gray] (6) at (5.25,0) {};
    \graph[edges={-Kite}]{(0) -> (1) -> (2) -> (4) -> (5), (6) -> (5) -> (4), (0) -> (2) -> (5), (1) -> (3) -> (5), (1) -> (5)};
    \onslide<6->{
        \foreach \i in {0,1,5,6} {\fill[@primary] (\i) circle [radius=4pt];}
        \node[above left] at(current bounding box.south east) {\faCheck};
    }
\end{tikzpicture}}\qquad\onslide<7->{\begin{tikzpicture}[scale=.5,gray]
    \node[ball,gray] (0) at (0,0) {};
    \node[ball,gray] (1) at (1,1.125) {};
    \node[ball,gray] (2) at (1,-1.125) {};
    \node[ball,gray] (3) at (2.5,1.125) {};
    ...
    \node[ball,gray] (5) at (3.75,0) {};
    \node[ball,gray] (6) at (5.25,0) {};
    \node[ball,gray] (7) at (5.25,1.125) {};
    \graph[edges={-Kite}]{(0) -> (1) -> (2) -> (4) -> (5), (6) -> (5) -> (4), (0) -> (2) -> (5), (1) -> (3) -> (5), (1) -> (5)};
    \onslide<8->{
        \foreach \i in {0,2,4,5,6} {\fill[@primary] (\i) circle [radius=4pt];}
        \fill[@secondary] (7) circle [radius=4pt];
        \node[above left] at(current bounding box.south east) {\faClose};
    }
\end{tikzpicture}}
\end{center}
        \item<9->[Pfad] Ein Weg, der die Kantenrichtung beachtet oder (je nach Definition) keinen Knoten doppelt enthält.\par\medskip\pause
\begin{center}
\onslide<10->{\begin{tikzpicture}[scale=.5,gray]
    \node[ball,gray] (0) at (0,0) {};
    \node[ball,gray] (1) at (1,1.125) {};
    ...
    \node[above left] at(current bounding box.south east) {\faCheck};
\end{tikzpicture}}\qquad\onslide<11->{\begin{tikzpicture}[scale=.5,gray]
    \node[ball,gray] (0) at (0,0) {};
    \node[ball,gray] (1) at (1,1.125) {};
    \node[ball,gray] (2) at (1,-1.125) {};
    \node[ball,gray] (3) at (2.5,1.125) {};
    ...
    \graph[edges={-Kite}]{(0) -> (1) -> (2) -> (4) -> (5), (6) -> (5) -> (4), (0) -> (2) -> (5), (1) -> (3) -> (5), (1) -> (5)};
    \onslide<11->{
        \foreach \i in {0,1} {\fill[@primary] (\i) circle [radius=4pt];}
        \foreach \i in {5,6} {\fill[@secondary] (\i) circle [radius=4pt];}
    }
    \node[above left] at(current bounding box.south east) {\faClose};
\end{tikzpicture}}\qquad\onslide<12->{\begin{tikzpicture}[scale=.5,gray]
    \node[ball,gray] (0) at (0,0) {};
    \node[ball,gray] (1) at (1,1.125) {};
    \node[ball,gray] (2) at (1,-1.125) {};
    \node[ball,gray] (3) at (2.5,1.125) {};
...
\begin{frame}{Graphen durchsuchen: Tiefensuche}
    ...
    \begin{multicols}{5}
        \scriptsize%
    \begin{enumerate}[<+(1)->]
        \item \begin{tikzpicture}[scale=.625,every node/.style={transform shape},kreis/.style={inner sep=4pt,ball,font=\sffamily,outer sep=1pt},ikreis/.style={inner sep=4pt,iball,font=\sffamily,outer sep=1pt},armark/.style={thick,@secondary},baseline=-.6ex]
...
\begin{frame}[c]{Ein Beispiel für die Tiefensuche}
\vspace*{1.5em}\par\centering
...
\savebox0{\scalebox{.5}{\begin{tikzpicture}
    \onslide<2->{\draw[thick,@primary,rounded corners=2.5pt,path picture={\node at(path picture bounding box.center) {\href{https://media.githubusercontent.com/media/EagleoutIce/Episode-Traversierung/gh-pages/noanim_traversal.pdf}{\includegraphics[width=8.5cm,height=4.788cm,keepaspectratio]{logoTraversal-\jobname.png}}};}] (0,0) rectangle (8.5cm,4.788cm);
    \node[below=1cm,yshift=4.5mm,scale=1.5] at (current bounding box.south) {\fancyqr[link]{https://github.com/EagleoutIce/Episode-Traversierung/tree/main}};}
\end{tikzpicture}}}\raisebox{.2\ht0}{\usebox0}%
\end{frame}
...
\begin{frame}{Graphen durchsuchen: Breitensuche}
    \begin{itemize}[<+(1)->]
        ...
        \item In diesem Zuge merkt man sich die Nachbarn dieser Knoten (mit einer \hyperlink{mrk:Queue}{Queue}) und besucht anschließend diese.
    ...
    \begin{multicols}{5}
        \scriptsize%
    \begin{enumerate}[<+(1)->]
        \item \begin{tikzpicture}[scale=.625,every node/.style={transform shape},kreis/.style={inner sep=4pt,ball,font=\sffamily,outer sep=1pt},ikreis/.style={inner sep=4pt,iball,font=\sffamily,outer sep=1pt},armark/.style={thick,@secondary},okreis/.style={inner sep=4pt,oball,font=\sffamily,outer sep=1pt},armark/.style={thick,@secondary},baseline=-.6ex]
    \node[ikreis] (a) at (0,0) {1};
...
\begin{frame}[c]{Ein Beispiel für die Breitensuche}
\vspace*{1.5em}\par\centering
\onslide<3->{\begin{tikzpicture}
\Matrix{%
{X,X,X,X,X,X,X,X,X,X},
...
\begin{frame}[fragile,c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[LinkedList - toString() \Time{4}]
        Gegeben sei folgender Code.\pause{} Es darf davon ausgegangen werden, dass \bjava{get(int)} korrekt implementiert ist.\pause{} Schreiben Sie eine Methode \bjava{toString()}, die die Elemente in korrekter Reihenfolge ausgibt.\par\pause{}
{\footnotesize
\begin{plainjava}
...
\end{plainjava}
}\pause{}Beispiel: \bjava{13, 12, -4, 9,} \textcolor{gray}{(Das endständige Komma ist erlaubt)}
    \end{exercise}
\end{frame}
...
!*\onslide<7->*!        current = current.next;
!*\onslide<6->*!    }
!*\onslide<8->*!    return strList;
!*\onslide<3->*!}
\end{plainjava}
        \onslide<9->{Ist kein Komma am Ende erwünscht, so genügt ein Test der Form \bjava{if(current.next != null)} für das Anfügen des Kommas.}
    \end{solve}
\end{frame}
...
\begin{plainjava}
!*\onslide<3->*!Element reverse(Element head) {
!*\onslide<4->*!    Element reversedHead = null;
!*\onslide<4->*!    Element current = head;
!*\onslide<5->*!    while (current != null) {
!*\onslide<6->*!        Element next = current.next;
!*\onslide<7->*!        current.next = reversedHead;
!*\onslide<8->*!        reversedHead = current;
!*\onslide<8->*!        current = next;
!*\onslide<5->*!    }
!*\onslide<9->*!    return reversedHead;
!*\onslide<3->*!}
\end{plainjava}
    \onslide<10->{Doch was passiert hier? Betrachten wir dies im Detail\ldots}
    \end{solve}
\end{frame}
...
\end{minipage}\endgroup\hfill\par
    \begin{enumerate}
        ...
        \item<4-> Nun zeigt \bjava{current.next} auf das vorherige Element:\smallskip\par \begin{tikzpicture}[scale=.6,every node/.append style={transform shape},baseline]
            ...
            \draw[-Kite,rounded corners=1pt] ($(b.east)+(-1em,0)$) -- ++(0,1) -| (a.north);
    ...
    \onslide<5->{\begin{center}
        \scriptsize\textit{Wie ein Schiffchen schieben wir also \T{reversedHead}, \T{current} und \T{next} nach rechts durch die Liste, um aus \(\text{\T{current}}\to\text{\T{next}}\) die Kante \(\text{\T{current}}\to\text{\T{reversedHead}}\) zu machen.}\vspace*{-2mm}
    \end{center}}
    \end{solve}
\end{frame}
...
\end{minipage}\endgroup\hfill\par
    \begin{enumerate}
        ...
        \begin{tikzpicture}[scale=.6,every node/.append style={transform shape},baseline]
            ...
            \draw[-Kite,rounded corners=1pt] ($(b.east)+(-1em,0)$) -- ++(0,1) -| (a.north);
            \draw[-Kite,rounded corners=1pt] ($(c.east)+(-1em,0)$) -- ++(0,.85) -| (b.north);
...
\begin{frame}[c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Theorie zu Stapel und Warteschlange \Time{5}]
        \pause{}Bewerten (wahr\thinspace /\thinspace falsch) und Begründen Sie jeweils: \begin{enumerate}[<+(1)->]
            ...
            \item Der Java-(Objekt-)Heap wird auch als (Datenstruktur) \emph{Heap} verwaltet.
        \end{enumerate}
    \end{exercise}
\end{frame}
...
\begin{frame}[c]{Lösung}
    \begin{solve}<2->[Theorie zu Stapel und Warteschlange]
        \pause{}\begin{enumerate}[<+(1)->]
            ...
            \item Java-Heap ist Heap:\pause{} Dies ist falsch, der Heap auf dem (komplexe) Daten abgelegt werden,\pause{} hat nur den selben Namen \textcolor{gray}{(siehe: \T{\href{https://stackoverflow.com/questions/756861/whats-the-relationship-between-a-heap-and-the-heap}{stackoverflow}}).}
        \end{enumerate}
    \end{solve}
\end{frame}
...
\begin{frame}[c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Traversierung eines Baumes \Time{4}]
        ...
        \begin{center}
            \scalebox{.55}{\begin{forest}
                for tree={inner sep=4pt,ball,font=\Large\sffamily,outer sep=1pt,execute at begin node={\strut},minimum width=1.2cm}
...
\begin{frame}[c]{Lösung}
    \begin{solve}<2->[Traversierung eines Baumes]
        \begin{center}
            \scalebox{.5}{\begin{forest}
                for tree={inner sep=4pt,ball,font=\Large\sffamily,outer sep=1pt,execute at begin node={\strut},minimum width=1.2cm}
                [A[B[,phantom][C[D][,phantom]]][E[F[,phantom][,phantom]][G[H][I]]]]
            \end{forest}}
        \end{center}
        \begin{enumerate}[<+(1)->]
            \item<3-> Breitendurchlauf (Level-Order): \(A,B,E,C,F,G,D,H,I\).
            \item<4-> In-Order: \(B,D,C,A,F,E,H,G,I\).
            \item<5-> Post-Order: \(D,C,B,F,H,I,G,E,A\).
        \end{enumerate}
    \end{solve}
\end{frame}

\begin{frame}[c]{Übungsaufgabe}
    \Task{Baum durch Traversierung}
    \begin{exercise}<2->[Baum durch Traversierung \Time{4}]
        \onslide<3->{Gegeben seien die beiden folgenden Traversierung des selben Binärbaums:}
        \begin{itemize}
            \item<4-> Pre-Order: \(9, 7, 14, 12, 0, 11, 8\)
            \item<4-> In-Order: \(14, 12, 7, 0, 9, 11, 8\)
        \end{itemize}
        \onslide<5->{Stellen Sie einen Binärbaum grafisch dar, welcher diese Ausgaben erzeugt. Achten Sie darauf deutlich zu zeichnen, ob es sich jeweils um linke oder rechte Kindknoten handelt.}
    \end{exercise}
\end{frame}
...
\begin{frame}[c]{Lösung}
    \begin{solve}<2->[Traversierung eines Baumes]
        ...
        \end{enumerate}
        \begin{center}
            \onslide<5->{\scalebox{.5}{\begin{forest}
                for tree={inner sep=4pt,ball,font=\Large\sffamily,outer sep=1pt,execute at begin node={\strut},minimum width=1.2cm,s sep=1.65em}
                [9[7[14[,phantom][12]][0[,phantom][,phantom]]][11[,phantom][8]]]
            \end{forest}}}
        \end{center}
        \onslide<6->{Wir sehen, dass \(9\) die Wurzel ist (erstes Element in Pre-Order), die Suche der \(9\) in In-Order liefert die Aufteilung auf links und rechts\ldots}
    \end{solve}
\end{frame}
...
\begin{frame}[fragile,c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Traversierung eines Baumes - Java \Time{4}]
        Die folgende Klasse definiere die Knoten eines Binärbaums.\pause{} Schreiben Sie eine rekursive Methode \bjava{inorder(Node n)}, die eine In-Order Traversierung ab dem Knoten \(n\) vornimmt.\pause{} Der Wert des Knoten soll (jeweils) durch \bjava{System.out.println} ausgegeben werden.\pause{}
\begin{plainjava}
public class Node {
    public int value;
...
\begin{frame}[fragile,c]{Übungsaufgabe}
    \Task{Traversierung eines Baumes, 2 - Java}
    \begin{exercise}<2->[Traversierung eines Baumes, 2 - Java \Time{6}]
        \bjava{Node} definiere die Knoten eines Binärbaums, \solGet{keywordC}{Stack} sei eine korrekt implementierte Stack-Klasse (deren Konstruktor einen leeren Stack initialisiert).\pause{} Schreiben Sie eine \imp{iterative} Methode \bjava{preorderIt(Node n)}, die eine Pre-Order Traversierung ab dem Knoten \(n\) vornimmt. Diese soll dabei den linken vor dem rechten Teilbaum bearbeiten.\pause{} Der Wert des Knoten soll (jeweils) durch \bjava{System.out.println} ausgegeben werden.\pause{}
\begin{columns}[c]
\lstfs{9}%
...
\begin{plainjava}[morekeywords={[3]{Stack}}]
!*\onslide<3->*!void preorderIt(Node n) {
!*\onslide<4->*!    Stack s = new Stack();
!*\onslide<5->*!    s.push(n);
!*\onslide<6->*!    while(!s.isEmpty()) {
!*\onslide<7->*!        Node current = s.pop();
!*\onslide<8->*!        if(current == null)
!*\onslide<8->*!            continue;
!*\onslide<9->*!        System.out.println(current.value);
!*\onslide<10->*!        s.push(current.right);
!*\onslide<11->*!        s.push(current.left); // erst links
!*\onslide<6->*!    }
!*\onslide<3->*!}
\end{plainjava}
    \end{solve}
...
\begin{frame}[c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Breitensuche - Pseudocode \Time{6}]
        \pause\label{task:bfs-pseudo}Schreiben Sie Pseudocode, der den folgenden Vertrag durch eine Breitensuche erfüllt:
\RestyleAlgo{tworuled}\smallskip\par\pause
\SetKwInput{KwIn}{In}\SetKwInput{KwOut}{Out}%
\SetAlgoVlined%
\begin{algorithm}[H]
\PreCode
...
\end{algorithm}\smallskip\par\pause
        Sie können Mengen, sowie Warteschlangen verwenden: \begin{itemize}
            \item \T{Queue.create()} erzeugt eine neue Warteschlange.
            \item \T{enqueue(<Wert>)} fügt einen Wert hinten an.
            \item \T{dequeue()} liefert und entfernt den vordersten Wert.
...
\begin{algorithm}[H]
\PreCode\StartCode
\onslide<4->{\(queue \leftarrow \T{Queue.create()}\)\;}
\onslide<4->{\(queue.\T{enqueue}(s)\)\Comment*[r]{Startknoten hinzufügen}}
\onslide<5->{\(visited \leftarrow \{s\}\)\Comment*[r]{Menge markiert bereits beachtete}}
\onslide<6->{\While(\Comment*[f]{Es gibt noch besuchbare Punkte}){\(queue.\T{size}() > 0\)}{
    \onslide<7->{\(current \leftarrow queue.\T{dequeue}()\)\Comment*[r]{Betrachte nächsten Knoten}}
    \onslide<8->{\lIf(\Comment*[f]{Ist er das Ziel?}){\(current = g\)}{\KwRet\say{Ja}}}
    \onslide<9->{\Comment{Durchsuche Adjazenzmatrix nach allen erreichbaren Knoten}
    \For{i = 0 \KwTo n - 1}{
        \onslide<10->{\If(\Comment*[f]{Es gibt eine Kante von \(current\) nach \(i\)}){\(a_{current, i} > 0\)} {
            \onslide<11->{\If(\Comment*[f]{Noch nicht besucht}){\(i \not\in visited\)}{
                \onslide<12->{\(queue.\T{enqueue}(i)\)\;
                \(visited \leftarrow visited \cup \{i\}\)\Comment*[r]{Füge zu besuchten hinzu.}}
            }}
        }}
    }}
}}
\onslide<13->{\KwRet\say{Nein}\;}
\end{algorithm}
    \end{solve}
\end{frame}
...
!*\onslide<4->*!    Queue queue = new Queue(); queue.enqueue(s);
!*\onslide<5->*!    Set visited = new Set();   visited.add(s);
