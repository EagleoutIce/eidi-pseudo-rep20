...
\begin{frame}[c]{Lösung}
    \begin{solve}<2->[Traversierung eines Baumes]
        ...
        \end{center}
        \begin{enumerate}[<+(1)->]
            \item Breitendurchlauf (Level-Order): \(A,B,E,C,F,G,D,H,I\).
...
\begin{frame}[c]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Baum durch Traversierung \Time{4}]
        ...
        \end{itemize}
        \onslide<6->{Stellen Sie einen Binärbaum grafisch dar, welcher diese Ausgaben erzeugt. Achten Sie darauf, deutlich zu zeichnen, ob es sich jeweils um linke oder rechte Kindknoten handelt.}
    \end{exercise}
\end{frame}
...
\begin{frame}[c]{Lösung}
    \begin{solve}<2->[Traversierung eines Baumes]
        \begin{enumerate}[<+(1)->]
            \item Pre-Order: \(\only<6->{\underline}{9}, 7, 14, 12, 0, 11, 8\)
            \item In-Order: \(14, 12, 7, 0, \only<6->{\underline}{9}, 11, 8\)
        \end{enumerate}
        \begin{center}
            ...
            \end{forest}}}
        \end{center}
        \onslide<6->{Wir sehen, dass \(9\) die Wurzel ist, die Suche in In-Order die liefert Aufteilung auf links und rechts\ldots}
    \end{solve}
\end{frame}
