...
\begin{frame}[c,fragile]{Übungsaufgabe}
    ...
    \begin{exercise}<2->[Fehler finden - Vererbung \Time{4}]
        \pause{}Erklären Sie alle Java-Kompilierzeit-Fehler. Die Klassen befinden sich jeweils in Dateien mit dem korrekten Namen und in dem \emph{selben} Verzeichnis:\pause{}
\lstfs{9}
\begin{plainjava}
public class A {
...
\begin{frame}[c,fragile]{Lösung}
    \begin{solve}<2->[Fehler finden - Vererbung]
        \begin{enumerate}[<+(1)->]
            \item \bjava{B} benötigt einen Konstruktor, da es für die Superklasse nur \bjava{A(int)} gibt.
